...
def merge_rank_files(lowfile,highfile,outfile,args,cutoffinfo):
  ...
  for line in open(lowfile):
    ...
    if gid in lower_gene_lfc:
      g_lfc="{:.5g}".format(lower_gene_lfc[gid])
    else:
      g_lfc=0.0
    gfile[gid]=[[gitem,g_lo,g_p,g_fdr,nline-1,g_goodsgrna,g_lfc]]
  maxnline=nline
  nline=0
  for line in open(highfile):
    field=line.strip().split()
    ...
    g_fdr=float(field[4])
    g_goodsgrna=int(field[5])
    if gid in higher_gene_lfc:
      g_lfc="{:.5g}".format(higher_gene_lfc[gid])
    else:
      g_lfc=0.0
    if gid not in gfile:
      logging.warning('Item '+gid+' appears in '+highfile+', but not in '+lowfile+'.')
      #gfile[gid]=[('NA',1.0,1.0,maxnline)]
      gfile[gid]=[[1.0,1.0,1.0,maxnline,0,0.0]] # note that gitem is not saved
    else:
      ...
      gfile[gid]+=[[g_lo,g_p,g_fdr,nline-1,g_goodsgrna,g_lfc]] # don't repeat the gitem
  # check whether some items appear in the first group, but not in the second group
  for (k,v) in gfile.iteritems():
    if len(v)==1:
